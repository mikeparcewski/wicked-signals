...
beforeEach(() => {
  ...
  try { fs.unlinkSync(dbPath); } catch { /* file may not exist yet */ }
...
describe('ingestText — full pipeline', () => {
  it('returns a signal_id and direct_outcome when LLM classifies as direct', async () => {
    ...
    (classifySignal as jest.Mock).mockResolvedValueOnce({
      result: {
        type: 'direct',
        crew_type: null,
        confidence: 0.75,
  ...
  it('returns degraded success when ANTHROPIC_API_KEY is not set', async () => {
    const { classifySignal } = require('../../src/classify');
    (classifySignal as jest.Mock).mockRejectedValueOnce(
      new Error('Error: ANTHROPIC_API_KEY is not set'),
    );

    const { ingestText, isStructuredError } = require('../../src/ingest');
    const result = await ingestText('test signal text');
    // Graceful degraded mode: not an error — signal is stored, WARN emitted
    expect(isStructuredError(result)).toBe(false);
    expect((result as { status: string }).status).toBe('pending');
    expect((result as { route_target: string }).route_target).toBe('pending');
  });
});
...
describe('store-first guarantee', () => {
  it('signal exists in DB even when classification fails', async () => {
    const { classifySignal } = require('../../src/classify');
    (classifySignal as jest.Mock).mockRejectedValueOnce(new Error('LLM API timeout'));
