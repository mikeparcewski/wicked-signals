...
import { determineRoutingPath, buildCrewLaunchCommand } from '../../src/route';
...
describe('determineRoutingPath', () => {
  describe('direct type always routes to direct_outcome', () => {
    it('routes direct type with high confidence to direct_outcome', () => {
      const path = determineRoutingPath({ type: 'direct', confidence: 0.9, degraded: false });
      expect(path).toBe('direct_outcome');
    });

    it('routes direct type with low confidence to direct_outcome', () => {
      const path = determineRoutingPath({ type: 'direct', confidence: 0.1, degraded: false });
      expect(path).toBe('direct_outcome');
    });

    it('routes direct type in degraded mode to direct_outcome', () => {
      const path = determineRoutingPath({ type: 'direct', confidence: 0.75, degraded: true });
      expect(path).toBe('direct_outcome');
    });
  });
  ...
  describe('crew-launch type with confidence thresholds', () => {
    it('routes confidence >= 0.85 to auto_launch_crew (non-degraded)', () => {
      const path = determineRoutingPath({ type: 'crew-launch', confidence: 0.85, degraded: false });
    ...
    it('routes confidence >= 0.85 to auto_launch_crew (degraded — unreachable in practice)', () => {
      ...
      const path = determineRoutingPath({ type: 'crew-launch', confidence: 0.85, degraded: true });
      expect(path).toBe('auto_launch_crew');
    });

    it('routes confidence 0.80 (v0.1 cap) to crew_idd in degraded mode', () => {
      const path = determineRoutingPath({ type: 'crew-launch', confidence: 0.80, degraded: true });
      expect(path).toBe('crew_idd');
    });

    it('routes confidence >= 0.60 to crew_idd', () => {
      const path = determineRoutingPath({ type: 'crew-launch', confidence: 0.70, degraded: false });
      expect(path).toBe('crew_idd');
    });

    it('routes confidence exactly 0.60 to crew_idd', () => {
      const path = determineRoutingPath({ type: 'crew-launch', confidence: 0.60, degraded: false });
      expect(path).toBe('crew_idd');
    });

    it('routes confidence < 0.60 to aggregate', () => {
      const path = determineRoutingPath({ type: 'crew-launch', confidence: 0.40, degraded: false });
      expect(path).toBe('aggregate');
    });

    it('routes confidence 0.0 to aggregate', () => {
      const path = determineRoutingPath({ type: 'crew-launch', confidence: 0.0, degraded: false });
      expect(path).toBe('aggregate');
    });
  });
  ...
  describe('garden-action type', () => {
    it('routes garden-action with high confidence to aggregate (not direct_outcome)', () => {
      const path = determineRoutingPath({ type: 'garden-action', confidence: 0.5, degraded: false });
      expect(path).toBe('aggregate');
    });
  });
...
describe('buildCrewLaunchCommand', () => {
  it('builds a valid crew launch command string', () => {
    const cmd = buildCrewLaunchCommand({
      crewType: 'analysis',
